...
#include <gl\glut.h>                // Заголовочные файлы для библиотеки OpenGL32
...
RECT rect_window_gl;			//прямоугольник окна opengl
Render render;
...
int InitGL( GLvoid )                // Все установки касаемо OpenGL происходят здесь
{
	...
	glClearColor(0.1f, 0.1f, 0.1f, 0.0f);          // Очистка экрана в черный цвет
...
BOOL CreateGLWindow( LPCWSTR title, int width, int height, int bits, bool fullscreenflag )
{
	...
	else
	{
		...
		dwStyle    =   WS_VISIBLE | WS_POPUP;        // Обычный стиль окна
	}

	AdjustWindowRectEx( &rect_window_gl, dwStyle, false, dwExStyle );      // Подбирает окну подходящие размеры
	...
	if( !( hWnd = CreateWindowEx(  dwExStyle,          // Расширенный стиль для окна
          ...
          0, 0,            // Позиция окна
          rect_window_gl.right - rect_window_gl.left,    // Вычисление подходящей ширины
          rect_window_gl.bottom - rect_window_gl.top,    // Вычисление подходящей высоты
      ...
      int    nCmdShow )        // Состояние отображения окна
{
	...
	//if( MessageBox( NULL, _T("Хотите ли Вы запустить приложение в полноэкранном режиме?"),  _T("Запустить в полноэкранном режиме?"), MB_YESNO | MB_ICONQUESTION) == IDNO )
	//{
	//	fullscreen = false;          // Оконный режим
	//}

	// Создать наше OpenGL окно
	if( !CreateGLWindow( _T("NeHe OpenGL окно"), W_WIDTH, W_HEIGHT, 32, /*fullscreen*/false ) )
	{
	...
	while( !done )                // Цикл продолжается, пока done не равно true
	{
		...
		else                // Если нет сообщений
		{
			...
			if( keys[VK_F1] )          // Была ли нажата F1?
			{
				...
				if( !CreateGLWindow( _T("NeHe OpenGL структура"), W_WIDTH, W_HEIGHT, 32, fullscreen ) )
				{
